...
const newTokenController = async (req: Request, res: Response, _next: NextFunction) => {
  try {
    const { currentUser, refreshToken } = req.body;
    if (!refreshToken) return res.status(401).send(tokenMessages.noRefreshTokenReceived);

    const isRefreshTokenExpired = verify(refreshToken, process.env.JWT_REFRESH_SECRET!);
    if (!isRefreshTokenExpired) return res.status(401).send(tokenMessages.refreshTokenExpired);
    ...
    if (!hasRefreshToken) return res.status(405).send(tokenMessages.noRefreshTokenFound);
    const isRefreshTokenValid = verify(
      hasRefreshToken.refreshToken as string,
      process.env.JWT_REFRESH_SECRET! as string,
    );
    if (!isRefreshTokenValid) return res.status(405).send(tokenMessages.refreshTokenExpired);
    ...
    console.error(errorMessage(newTokenController.name, "line_26", error));
