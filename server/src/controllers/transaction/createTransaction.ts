...
const createTransactionController = async (req: Request, res: Response, _next: NextFunction) => {
  try {
    ...
    if (!saveNewTransaction) {
      console.error(errorMessage(createTransactionController.name, "line_26"));
      return res.status(500).send(statusMessages.internalerror);
    }

    return res.status(201).send(transactionMessages.createTransaction.creationSuccessfull);
  } catch (error) {
    console.error(errorMessage(createTransactionController.name, "line_32", error));
    return res.status(500).send(statusMessages.internalerror);
  }
};
